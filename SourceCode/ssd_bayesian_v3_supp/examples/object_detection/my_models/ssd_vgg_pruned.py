...
BASE_NUM_OUTPUTS = {
    300: [32, 62, 'M', 103, 123, 'M', 239, 240, 245, 'C', 438, 512, 512, 'M', 512, 512, 486],
...
class SSD_VGG_pruned(nn.Module):
    def __init__(self, cfg, size, num_classes, batch_norm=False):
        super(SSD_VGG_pruned, self).__init__()
        self.config = cfg
...
def build_vgg_ssd_layers(num_outputs, output_inddices, start_input_channels=3, batch_norm=False):
    ...
    for i, out_planes in enumerate(num_outputs):
        if out_planes in ('M', 'C'):
            ...
            vgg_layers.append(nn.MaxPool2d(kernel_size=2, stride=2, padding=0))
            continue
    ...
    vgg_layers.extend(make_ssd_vgg_layer(in_planes, 615, kernel=3, padding=6, dilation=6, batch_norm=batch_norm))
    vgg_layers.extend(make_ssd_vgg_layer(615, 1024, kernel=1, batch_norm=batch_norm))
...
def build_ssd_vgg_pruned(cfg, size, num_classes, config):
    ssd_vgg_pruned = SSD_VGG_pruned(cfg, size, num_classes, batch_norm=True)
    return ssd_vgg_pruned
