...
from my_nncf.utils import is_main_process, is_dist_avail_and_initialized, get_world_size
...
def predict_detections(data_loader, device, net):
    ...
    for batch_ind, (ims, _gts, hs, ws) in enumerate(data_loader):
        ...
        # timer.tic()
        logger.info('Detect for batch: {:d}/{:d}'.format(batch_ind + 1, num_batches))
        batch_detections = net(x,test_time=True)
...
def test_ir_net(batch_detections,data_loader):
    all_boxes = convert_detections(batch_detections)
    print(all_boxes)
    logger.info('Evaluating detections')
    return evaluate_detections(all_boxes, data_loader.dataset)