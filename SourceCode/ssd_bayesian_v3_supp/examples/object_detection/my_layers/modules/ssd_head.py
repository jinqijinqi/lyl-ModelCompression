...
from examples.object_detection.my_layers import DetectionOutput, PriorBox
from my_nncf.dynamic_graph.context import no_nncf_trace
    ...
    def __init__(self, num_input_features, num_classes, config):
        ...
        for i, num_features in enumerate(num_input_features):
            self.heads.append(SSDHead(
                num_features, num_classes, self.config.min_sizes[i], self.config.max_sizes[i],
                self.config.aspect_ratios[i], self.config.steps[i], self.config.variance, self.config.flip, self.config.clip
            ))
