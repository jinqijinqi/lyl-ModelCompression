...
class _ConvNdGroupNJ(Module):
    ...
    def compute_posterior_params(self):
        weight_var, z_var = self.weight_logvar.exp(), self.z_logvar.exp()
        # tt = self.z_mu.pow(2).reshape(self.z_mu.size(0),1,1,1)
        self.post_weight_var = self.z_mu.pow(2).reshape(self.z_mu.size(0),1,1,1) * weight_var + \
                               z_var.reshape(z_var.size(0),1,1,1) * self.weight_mu.pow(2) + \
                               z_var.reshape(z_var.size(0),1,1,1) * weight_var
        self.post_weight_mu = self.weight_mu * self.z_mu.reshape(self.z_mu.size(0),1,1,1)
        return self.post_weight_mu, self.post_weight_var
...
class Conv2dGroupNJ(_ConvNdGroupNJ):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, groups=1, bias=True,
                 cuda=True, init_weight=None, init_bias=None, clip_var=None):
    ...
    def forward(self, x):
        ...
        return reparametrize(mu_activations * z, (var_activations * z.pow(2) + self.epsilon).log(), sampling=self.training,
                             cuda=self.cuda)
